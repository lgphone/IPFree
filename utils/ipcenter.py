...
def exec_query_ip(ip='127.0.0.1'):
    ret = {'status': True,
           'data': ''}
    query_sql = "SELECT * FROM `aiwen_free_district_v2.0.1` WHERE minip <= INET_ATON('{_query_ip}') ORDER BY minip DESC LIMIT 1;".format(_query_ip=ip)
    try:
...
def exec_count_ip(ip='127.0.0.1'):
    ...
    query_sql = "SELECT * FROM `user_ip_count` WHERE ip = '{_c_ip}';".format(_c_ip=ip)
    update_sql = "UPDATE `user_ip_count` SET ip_count=ip_count+1 where ip = '{_c_ip}';".format(_c_ip=ip)
